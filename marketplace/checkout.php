...
// Check for error messages from process_checkout.php
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['checkout'])) {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // For guest users, use a placeholder user_id
        $user_id = 'guest';
    }
    
    // Create order in database
    $order_sql = "INSERT INTO order_details (user_id, total) VALUES (?, ?)";
    $order_stmt = $conn->prepare($order_sql);
    $order_stmt->bind_param("sd", $user_id, $total);
    ...
    if ($order_stmt->execute()) {
        ...
        // Insert payment details (assuming cash on delivery for now)
        $payment_sql = "INSERT INTO payment_details (order_id, amount, provider, status) VALUES (?, ?, ?, ?)";
        $payment_stmt = $conn->prepare($payment_sql);
        $amount = $total;
        $provider = "Cash on Delivery";
        $status = "Pending";
        
        $payment_stmt->bind_param("idss", $order_id, $amount, $provider, $status);
        $payment_stmt->execute();
        $payment_id = $conn->insert_id;
        $payment_stmt->close();
        
        // Update order with payment_id
        $update_order_sql = "UPDATE order_details SET payment_id = ? WHERE id = ?";
        ...
        header("Location: complete_order.php?order_id=" . $order_id);
        exit();
    } else {
        ...
        <div class="w-50 flex-auto " id="left">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <div class="mb-4 flex gap-[10px] mt-5">
                    ...
                    -2 w-full p-2 flex-1/2 focus:border-green-500 focus:outline-none" required placeholder="Last Name" value="<?php echo $lname;?>"/>
                </div>
                <div class="mb-4 gap-[10px]">
                    <input type="email" name="email" id="email" class="border-gray-500 rounded-xl shadow border
                    -2 w-full p-2 focus:border-green-500 focus:outline-none" required placeholder="Email" value="<?php echo $email;?>"/>
                </div>
                <div class="mb-4 gap-[10px]">
                    <input type="text" name="phone" id="phone" class="border-gray-500 rounded-xl shadow border
                    -2 w-full p-2 focus:border-green-500 focus:outline-none" required placeholder="Phone Number" value="<?php echo $phone;?>"/>
                </div>
                ...
                <div class="mb-4 gap-[50px] mt-[20px]">
                    ...
                    -2 w-full p-2 focus:border-green-500 focus:outline-none" required placeholder="Street, Apartment" value="<?php echo $street_address;?>"/>
                    <input type="text" name="address" id="address" class="border-gray-500 rounded-xl shadow-sm focus:shadow-md border
                    -2 w-full p-2 focus:border-green-500 focus:outline-none mt-[10px]" required placeholder="Address" value="<?php echo $address_line2;?>"/>
                    ...
                    -2 w-full p-2 flex-1/2 focus:border-green-500 focus:outline-none" required placeholder="City" value="<?php echo $city;?>"/>
                    <input type="text" name="state" id="state" class="border-gray-500 rounded-xl shadow-sm focus:shadow-md border
                    -2 w-full p-2 flex-1/2 focus:border-green-500 focus:outline-none" required placeholder="State" value="<?php echo $state;?>"/>
                    </div>

                    <div class="mb-4 flex gap-[10px]">
                    <input type="text" name="zip" id="zip" class="border-gray-500 rounded-xl shadow border
                    -2 w-full p-2 flex-1/2 focus:border-green-500 focus:outline-none focus:shadow-md" required placeholder="Zip Code" value="<?php echo $zip;?>"/>
                ...
                <button type="submit" name="checkout" class="bg-green-950 text-white p-3 shadow-green-600 rounded-xl flex gap-2.5">Checkout
